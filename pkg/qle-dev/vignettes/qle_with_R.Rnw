...
constructs a quasi-likelihood approximation model for estimating the model
parameter.\par
%
...
simulated statistics under the null hypothesis, exceeding the observed value of
...
values of the test statistic. In our implementation, we also allow the test
statistic to be different from the criterion function which was first used to
