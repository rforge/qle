...
doInParallel <- function(X, FUN, ... , fun = "lapply", cl = NULL,
							cache = getOption("qle.cache",FALSE) )
{
	...
	SIM <- if(cache) {
			...
			tryCatch({
			  function(x,...) {			
				...
				cache.fn <- sprintf("cached_%s.Rdata",dg)				
	...
	tryCatch({
		if(!is.null(cl)) {
			m <- min(length(X),length(cl))
			if(isTRUE(all.equal(fun,parallel::mclapply)) && m <= getOption("mc.cores",2L)) {
			 	fun(X, SIM, ...)		    
		    } else {
			  if(any(class(cl) %in% c("MPIcluster","SOCKcluster","cluster"))) {
				parallel::parLapplyLB(cl[seq_len(m)], X = X, fun = SIM, ...)
		...
		msg <- .makeMessage("Calling `",deparse(substitute(SIM)), "` failed: ", conditionMessage(e))
		return(structure(
				   list(message=msg, call=sys.call()),
				class = c("error", "condition"), error = e) )
	}
  )
}
...
#' @param fun			parallel option, "\code{lapply}" (default) or "\code{mclapply}" for parallel processing 
...
#' @export
simQLdata <-
  function(sim, ..., nsim, X = NULL, mode = c("list","matrix","mean"),
...
varCHOLdecomp <- function(matList, fun = "lapply") {
	# n <- NROW(matList[[1]])
	# lx <- n*(n+1)/2
	fun <- match.fun(fun)
	...
	fun(1:length(matList),doIt,x=matList)
}
...
#'
#' @description Calculate the data for estimation 
#'
#' @param runs		list or matrix of simulation results (see \code{\link{simQLdata}})
#' @param X			list or matrix of model parameters
...
#'
#' @details
#'  The function aggregates all neccessary data for QL estimation storing the
...
#'  If these fail for any reason we try to ignore, if possible, the corresponding sample point and parameter, exclude
...
#' @rdname 	setQLdata
#' @export
setQLdata <- function(runs, X = NULL, chol=TRUE, na.rm = TRUE,
						fun = getOption("qle.fork","lapply"), verbose = FALSE) {		
	nErrors <- 0
	nWarnings <- 0	
	nsim <- attr(runs,"nsim")
	...
	.extract <-  function(dataT) {
		...
		if(is.list(dataT))
		 dataT <- do.call(rbind,dataT)
	  	if(na.rm)
		 dataT <- dataT[!rowSums(is.na(dataT))>0,,drop=FALSE]
	...
	if(!is.null(nIgnored) && nrow(nIgnored)>0){
	...
	cvmats <- 
	 if(chol){
	    ...
	    if(verbose)
	 	...
	 	vmats <- fun(res[ok],"[[","V" )
		L <- try(varCHOLdecomp(vmats,fun),silent=TRUE)
		if(.isError(L)) {
