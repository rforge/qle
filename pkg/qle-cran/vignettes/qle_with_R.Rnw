...
estimation error by quasi-likelihood is due to the inherent simulation variance
and approximation error of $\bar{y}$ by kriging. This effect can be best exemplified by
a short simulation study.\par
%
...
\end{tabular}
\caption{Empirical mean square error (MSE) and average variance of the
re-estimated parameters for $100$ observations by our quasi-likelihood (qle) approach and
