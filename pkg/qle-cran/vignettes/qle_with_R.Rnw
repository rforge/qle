...
@
We could also apply the quasi-scoring iteration setting the following stopping
conditions.
<<eval=FALSE>>=
qs.opts <-
 list("xscale"=c(10,0.1,1),"ftol_abs"=1e-4,"score_tol"=1e-4)
@
<<>>=
qscoring(qsd,x0,opts=qs.opts,cvm=cvm)
@
...
approximations. The estimation is started by:
<<eval=FALSE>>=
