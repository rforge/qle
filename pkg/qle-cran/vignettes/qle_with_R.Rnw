...
The package provides two built-in types of parallel processing all
computations and model simulations based on the \pkg{parallel} package. Both
types can be combined for application of \pkg{qle} in a HPC environment of a
number of compute nodes or CPUs. First, computations  can be run in parallel by
spawning/forking tasks to multiple cores of a single system (see
\code{mclapply}\footnote{Note that \code{mclapply} is not available on
Windows-based platforms due to OS limitations, however, then run by
\code{lapply}.}) or distributing the computations (including the computational expensive model simulations)
to compute nodes of a HPC using a number of local CPUs at each node as a kind of
nested parallel processing. The user can pass any valid cluster object of class
\code{"MPIcluster","SOCKcluster","cluster"} which support
calling \code{parallel::parLapply()} to functions of the package
\pkg{qle}. Note that no computations are done in parallel by default since this
gives the most flexibility to use the package for all kinds of simulation
studies. For details of how to choose different options for parallel
computations please see the manual of the function \code{qle}.
%
...
\code{obs0}.
<<>>=
# use a local cluster object
options(mc.cores=2L) 
options(qle.multicore="lapply") # redundant
...
<<>>=
set.seed(123)
# again: use a local cluster object
options(mc.cores=2L)
# for multicore parallel processing instead of a cluster object:
