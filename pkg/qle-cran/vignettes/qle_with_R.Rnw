...
of evaluation points stored in \code{OPT}.
<<>>=
rbind("Var"=c("test"=attr(Stest,"aiqm"),"study"=qle.var[2]),
	  "MSE"=c(as.vector(Stest$param["RMSE"]^2),qle.var[1]))
@
Thus the variance estimation from testing is a ''pessimistic`` approximations of
the one derived from the simulation, see Table \ref{tab:err}. 
%
