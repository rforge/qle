...
estimates and the average variance denoted by $\overline{Var}$.
<<>>=
mle <- do.call(rbind,
...
errors for both methods. The predicted errors $\overline{Var}$ compare well to
the corresponding MSEs.\par
%, where the resulting errors for $\hat{\rho}_{mle}$
%suggest a lower error for the quasi-likelihood estimate. This is due
%to the additional simulation variance included in the computation of the
%variance matrix approximation of the statistics (see Section
%\ref{subsec:varianceInter}) by our approach.\par
%
Finally, we can apply the MC goodness-of-fit test of the estimated
parameter based on the same observations \code{obs0}.
<<>>=
Stest <- qleTest(OPT,sim=simfn,cond=cond,obs=obs0)
Stest
@
which also shows a good match of the empirical and predicted error
measures. Note that both errors resulting from the test are not the same as
shown in Table \ref{tab:err} since the test procedure does not use new evaluation
points during the re-estimation given the (generated) observed statistics. Thus
the results from the test are only based on the fixed set of evaluation points
as the result of the estimation in the object \code{OPT}.
%
