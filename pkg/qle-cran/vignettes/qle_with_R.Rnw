...
candidates during the local phase of the algorithm we adapt the idea proposed
in \citet{ref:Regis2007} to our setting. Let
\begin{equation}
...
\begin{equation}\label{stilde}
 \tilde{s}_n(\theta) = 
  ...
  \end{cases}
\end{equation}
denotes its normalized value for all $\theta\in\Omega^l_n$ where we compute the minimum and maximum
of the modified QD by
...
QS <- qscoring(OPT$qsd,OPT$par,
       ...
       cvm=OPT$cvm)
@
The estimated parameters are, however, not much of an improvement
<<>>=
