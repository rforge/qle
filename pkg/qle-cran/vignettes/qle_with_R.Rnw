...
\end{equation*}
is the expectation of $N$ a function of $\rho<1$. Hence the parameter of interest for estimation
is $\theta=\rho$, which can be interpreted as the fraction of time the server is
...
@
We set the ''real`` observation of the statistic $Y$ to \code{obs=1}
...
and its (numerically evaluated) observed analogue. The results suggest a
consistent root of the quasi-score and thus a plausible estimate of the unknown
parameter. We can also perform a MC goodness-of-fit test based on the final
approximation of the summary statistic,
<<>>=
Stest <- qleTest(OPT,sim=simfn,nsim=100,iseed=1234)
@
which shows the predicted error in the column \code{Estimate} (in the last
line), as the square root of the inverse of the predicted quasi-information.
<<>>=
Stest
@
%
...
form we can derive the \emph{maximum likelihood estimator} (mle),
that is,
\[
...
\]
In this case the score function is identical to the quasi-score function,
\[
...
\]
%
Therefore, compared to the maximum liklihood method, the resulting error of the
quasi-likelihood estimate $\hat{\rho}$ is due to the inherent simulation variance
and approximation error of $\bar{y}$ by kriging. This effect can be best
exemplified by a short simulation study.\par
%
We generate some observations at a fixed ''true`` parameter value, say
$\rho=0.7$, using $n=25$ time points.
% short simulation study
<<>>=
tet0 <- c("rho"=0.7)
obs0 <- simQLdata(sim=simfn,nsim=100,X=tet0)
@
and compute the mean squared error (MSE) of the maximum likelihood
estimate and the average variance.
<<>>=
mle <- do.call(rbind,
...
@
Now, based on the same observations, we estimate the parameter by our
quasi-likelihood approach using the initial design.
<<>>=
OPTS <- lapply(obs0[[1]],
...
\end{tabular}
\caption{Mean square error (MSE) and average variance of the re-estimated
parameters for $100$ observations by our quasi-likelihood (qle) approach and
maximum likelihood method.}
\label{tab:err}
\end{table}
%
%
The results are shown in Table \ref{tab:err}.
%
